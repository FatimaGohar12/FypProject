            ...
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>Posts</h1>
                            <a href="./postinsert.php" class="btn btn-primary my-3">Create New</a>
                                            ...
                                            <div class="card-body">
                                                <table class="table table-striped table-hover" id="myTable"
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <?php
                  // $sql = "SELECT posts.*,users.* FROM posts LEFT JOIN users ON users.sno=posts.post_user_id";
                  $sql = "SELECT posts.*, 
                  CASE WHEN posts.is_admin = false THEN users.name ELSE admin.`ad-username` END AS admin_name
                  FROM posts
                  LEFT JOIN users ON users.sno = posts.post_user_id AND posts.is_admin = false
                  LEFT JOIN admin ON admin.`Admin-id` = posts.post_user_id AND posts.is_admin = true";
                  ...
                  while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $posted_by = $row['admin_name'];
                    $post_id = $row['post_id'];
                    //    var_dump($post_id = $row['post_id']);
                    ...
                    <td>" . substr($content, 0, 40) . "</td>
                    
                    
                    
                    <td>
                    <form action='./postupdate.php' method ='GET'>
...
<input type='button' id='' name ='' value='Delete' data-id='" . $post_id . "' class='btn-delete-post btn btn-danger'>
