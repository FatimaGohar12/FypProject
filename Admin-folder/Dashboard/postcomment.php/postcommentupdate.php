...
$sql = "SELECT postscomment.*,posts.*,users.* FROM postscomment LEFT  JOIN posts ON posts.post_user_id=postscomment.comment_user_id LEFT JOIN users on users.sno = postscomment.comment_user_id";
...
$updatename = $row['name'];
$updatetitle = $row['post_title'];
$updatecontent = $row['post_content'];
$updatecomment = $row['comment_content'];
            ...
            <section class="content">
                <form action="" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="" rows="4" style="height: 209px;" name="content"><?php echo "$updatecomment" ?></textarea>
                    </div>
...
if (isset($_POST['submit'])) {
    // $thread = $_POST['thread'];
    //    var_dump($name = $_POST['posted_by']);
    var_dump($content = $_POST['content']);
    $title = $_POST['title'];
    ...
    $sql = "UPDATE `postscomment` SET `comment_content` = '$content' WHERE `postscomment`.`comment_id` = $comment_id ";
    $result = mysqli_query($conn, $sql);
    ...
    if ($result) {
        echo "
         ...
         Swal.fire({
            ...
            window.location.href = './postcommenthandel.php';
        })
        </script>
        ";
    } else {
